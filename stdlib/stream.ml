...
and buffio =
  { ic : in_channel; buff : bytearray; mutable len : int; mutable ind : int }
;;
exception Failure;;
...
let fill_buff b =
  b.len <- input b.ic b.buff 0 (Bytearray.length b.buff); b.ind <- 0
;;
...
let rec get_data count d = match d with
 ...
 | Sbuffio b ->
     ...
     if b.len == 0 then Sempty else
       let r = Obj.magic (Bytearray.unsafe_get b.buff b.ind) in
       (* Warning: anyone using g thinks that an item has been read *)
...
let rec peek s =
 ...
 | Sbuffio b ->
     ...
     else Some (Obj.magic (Bytearray.unsafe_get b.buff b.ind))
;;
...
let of_channel ic =
  {count = 0;
   data = Sbuffio {ic = ic; buff = Bytearray.create 4096; len = 0; ind = 0}}
;;
