...
val sub : bytearray -> pos:int -> len:int -> bytearray
...
val blit :
  src:bytearray -> src_pos:int -> dst:bytearray -> dst_pos:int -> len:int
  -> unit
...
val concat : sep:bytearray -> bytearray list -> bytearray
(** [String.concat sep sl] concatenates the list of strings [sl],
   inserting the separator string [sep] between each. *)

val iter : f:(char -> unit) -> bytearray -> unit
(** [String.iter f s] applies function [f] in turn to all
   the characters of [s].  It is equivalent to
   [f s.[0]; f s.[1]; ...; f s.[String.length s - 1]; ()]. *)

val iteri : f:(int -> char -> unit) -> bytearray -> unit
...
val map : f:(char -> char) -> bytearray -> bytearray
