...
val sub : 'a _string -> pos:int -> len:int -> string
...
val blit :
  src:'a _string -> src_pos:int -> dst:bytearray -> dst_pos:int -> len:int
  -> unit
...
val concat : sep:'a _string -> 'b _string list -> string
(** [String.concat sep sl] concatenates the list of strings [sl],
   inserting the separator string [sep] between each. *)

val iter : f:(char -> unit) -> 'a _string -> unit
(** [String.iter f s] applies function [f] in turn to all
   the characters of [s].  It is equivalent to
   [f s.[0]; f s.[1]; ...; f s.[String.length s - 1]; ()]. *)

val iteri : f:(int -> char -> unit) -> 'a _string -> unit
...
val map : f:(char -> char) -> 'a _string -> string
