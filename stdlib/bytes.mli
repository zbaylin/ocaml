...
(** Byte sequence operations.
  ...
  sequence of bytes. Each byte can be indexed in constant time for
  reading or writing.

  Given a byte sequence [s] of length [l], we can access each of
  the [l] bytes of [s] via its index in the sequence. Indexes start
  at [0], and we will call an index valid in [s] if it falls within the
  range [[0...l-1]]. A position is the point between two bytes or at
  the beginning or end of the sequence.  We call a position valid
  in [s] if it falls within the range [[0...l]]. Note that the byte at
  ...
  {!Bytes.set} and {!Bytes.blit} functions described below.
  See also strings, which are almost the same data structure, but
...
(** [set s n c] modifies [s] in place, replacing the byte at index
   number [n] with the byte [c].
...
(** [make n c] returns a fresh byte sequence of length [n],
...
(** Return a copy of the given byte sequence. *)
...
(** Return a new string that contains the same bytes as the given
    byte sequence. *)

val sub : bytes -> int -> int -> bytes
(** [sub s start len] returns a new byte sequence of
   length [len], containing the subsequence of [s] that
...
(** [blit src srcoff dst dstoff len] copies [len] bytes
   from sequence [src], starting at index [srcoff], to
   sequence [dst], starting at index [dstoff]. It works
   correctly even if [src] and [dst] are the same byte sequence,
...
(** [iter f s] applies function [f] in turn to all
   the bytes of [s].  It is equivalent to
   [f (get s 0); f (get s 1); ...; f (get s (length s - 1)); ()]. *)

val iteri : (int -> char -> unit) -> bytes -> unit
(** Same as {!String.iter}, but the
   function is applied to the index of the byte as first argument
   (counting from 0), and the byte itself as second argument.
*)

val map : (char -> char) -> bytes -> bytes
(** [map f s] applies function [f] in turn to all
   the bytes of [s] and stores the resulting bytes in a new
   sequence that is returned as the result. *)

val trim : bytes -> bytes
(** Return a copy of the argument, without leading and trailing
   whitespace. The bytes regarded as whitespace are: [' '],
   ['\012'], ['\n'], ['\r'], and ['\t']. *)
...
(** [index_from s i c] returns the index of the
   first occurrence of byte [c] in [s] after position [i].
...
(** [rindex_from s i c] returns the index of the
   last occurrence of byte [c] in [s] before position [i+1].
...
(** [contains_from s start c] tests if byte [c]
...
(** [rcontains_from s stop c] tests if byte [c]
