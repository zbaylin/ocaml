...
type sophisticated_display = {
  ...
  mutable ds_jobs_cached     : int;                    (** Number of jobs cached *)
          ds_tagline         : bytearray;              (** Current tagline *)
...
(* ***)
(*** create_tagline *)
let create_tagline description = Bytearray.make (List.length description) '-';;
(* ***)
(*** create *)
let create
  ?(channel=stdout)
...
let redraw_sophisticated ds =
  ...
  fp oc "%a %-4d (%-4d) %a %s %s %s"
    ...
    (Bytearray.to_string ds.ds_tagline)
    ticker;
...
let update_tagline_from_tags ds =
  ...
  let rec loop i = function
    | [] ->
        for j = i to Bytearray.length tagline - 1 do
          Bytearray.set tagline j '-'
        done
    | (tag, c) :: rest ->
        if Tags.mem tag tags then
          Bytearray.set tagline i (Char.uppercase c)
        else
          if Tags.mem tag ds.ds_seen_tags then
            Bytearray.set tagline i (Char.lowercase c)
          else
            Bytearray.set tagline i '-';
        loop (i + 1) rest
  in
  loop 0 ds.ds_tld;
;;
