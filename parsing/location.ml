...
let highlight_terminfo ppf num_lines lb locs =
  ...
  for i = pos0 to lb.lex_buffer_len - 1 do
    if Bytearray.get lb.lex_buffer i = '\n' then incr lines
  done;
  ...
  for pos = 0 to lb.lex_buffer_len - pos0 - 1 do
    ...
    let c = Bytearray.get lb.lex_buffer (pos + pos0) in
    print_char c;
    bol := (c = '\n')
  done;
...
let highlight_dumb ppf lb loc =
  ...
  for pos = 0 to end_pos do
    if Bytearray.get lb.lex_buffer (pos + pos0) = '\n' then begin
  ...
  for pos = 0 to end_pos do
    match Bytearray.get lb.lex_buffer (pos + pos0) with
    | '\n' ->
