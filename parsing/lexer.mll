...
let initial_string_buffer = Bytearray.create 256
...
let store_string_char c =
  if !string_index >= Bytearray.length !string_buff then begin
    let new_buff = Bytearray.create (Bytearray.length (!string_buff) * 2) in
    Bytearray.blit !string_buff 0 new_buff 0 (Bytearray.length !string_buff);
    string_buff := new_buff
  end;
  Bytearray.unsafe_set !string_buff !string_index c;
  incr string_index
...
let get_stored_string () =
  let s = Bytearray.sub_string !string_buff 0 !string_index in
  string_buff := initial_string_buffer;
  s
...
let remove_underscores s =
  ...
  let a = Bytearray.create l in
  let rec remove src dst =
    if src >= l then
      if dst >= l then s else Bytearray.sub_string a 0 dst
    else
      match s.[src] with
        '_' -> remove (src + 1) dst
      |  c  -> Bytearray.set a dst c; remove (src + 1) (dst + 1)
  in remove 0 0
